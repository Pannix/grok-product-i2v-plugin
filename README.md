...
```text
商品图 ──┐
内容物图 ──┼─> Grok 商品图生视频节点 ──> 脚本与分镜首帧图 ──> Grok 逐镜头视频
人物图 ──┼─> Grok 商品图生视频节点 ──> 脚本与分镜首帧图 ──> Grok 逐镜头视频
风格图 ──┘                         └─> 质检清单
```

0.6.0 把参考图扩展为商品、内容物、人物、风格四组，每组最多 3 张。分镜图先用图生图模型生成并落到画布，视频阶段再把每张分镜图作为 Grok image-to-video 的实际首帧，按镜头逐个生成。
...
- 商品参考图、内容物参考图、人物参考图、风格参考图分别支持 1–3 张；未使用的组填写 0。
- 参考图连接和提示词顺序统一为：商品 → 内容物 → 人物 → 风格。所有已分组图片都会传给分镜图模型，内容物不会再被误当成人物或风格图。
- 可以在节点中填写四组数量；如果不填写数量，旧节点继续兼容原来的顺序规则。图片节点标题包含“内容物”“人物”“风格”等关键词时会自动识别角色。
- 每张分镜图的提示词都会写明四组参考图的职责：商品锁定包装和可见文字，内容物只确认可见内部/随附内容，人物锁定脸型与身份，风格只借鉴氛围，不允许替换商品。
- 超过每组 3 张或数量没有覆盖的图片会标记为“未分组”，仍作为补充参考传入，但建议补齐数量或重命名图片节点。
...
1. 把商品首帧图片上传到画布。
2. 创建本节点，把商品、内容物、人物和风格图片节点都连到本节点。连接顺序为商品 → 内容物 → 人物 → 风格；每组可连接 1–3 张，然后在节点里填写四组数量。图片标题包含角色关键词时会自动识别。
3. 在「视频目标 / 主动作」里写你想要的效果，例如：`25 岁亚洲女性拿起这包姜枣茶，面对镜头介绍，最后把包装正面展示清楚；动作自然，商品不变形。`
4. 选择时长、画幅、分镜数和「人物一致性」模式。要锁人物脸型，选择「人物锁定」并填写人物参考图数量，确保人物参考图确实是同一个人；没有人物图时不要选择该模式。
...
分镜阶段的参考图分组顺序由插件固定为：商品、内容物、人物、风格，每组最多 3 张。图像模型是否能理解多图语义仍取决于模型接口；提示词会重复写出角色分工，但不等于模型能力保证。
